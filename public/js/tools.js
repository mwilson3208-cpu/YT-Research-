...
export const TOOLS = [
  {
    ...
    render(data) {
      ...
      if (data.autocompleteSource === 'offline-bank') {
        nodes.push(notice('warn', 'Using the offline keyword bank', 'YouTube autocomplete could not be reached from this machine (firewall or no connection). Results are pattern-generated instead of live.'));
      }
      nodes.push(stats([
      ...
      nodes.push(panel('All keywords', `${data.keywords.length} rows - sort, filter and export`, [
        createTable({ columns, rows: data.keywords, filename: `keywords-${data.seed.replace(/\s+/g, '-')}.csv`, storageKey: 'ta:cols:keywords' }),
      ]));
...

  {
    ...
    render(data) {
      ...
      nodes.push(panel('Side by side', `${good.length} channels`, [
        createTable({ columns, rows, filename: 'channel-comparison.csv', storageKey: 'ta:cols:compare' }),
      ]));
...

  {
    ...
    render(data) {
      ...
      nodes.push(panel('All comments', '', [
        createTable({ columns, rows: data.comments, filename: `comments-${data.videoId}.csv`, storageKey: 'ta:cols:comments' }),
      ]));
      return nodes;
    },
  },

  {
    ...
    render(data) {
      ...
      nodes.push(panel('Every tag found', 'Sorted by how many videos use it', [
        ...
        createTable({ columns, rows: data.tags, filename: 'tags.csv', storageKey: 'ta:cols:tags' }),
      ]));
      nodes.push(el('div', { class: 'two-col' }, [
    ...
    render(data) {
      ...
      return [
        ...
        panel('Generated titles', 'Sorted by score. Click Copy to take the whole list.', [
          ...
          createTable({ columns, rows: data.titles, filename: 'titles.csv', storageKey: 'ta:cols:titles' }),
        ]),
